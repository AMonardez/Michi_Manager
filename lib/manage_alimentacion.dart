...
class ManejaAlimentacion extends StatefulWidget {
  final Animal a;
  ManejaAlimentacion(this.a);
  @override
  _ManejaAlimentacionState createState() => new _ManejaAlimentacionState(a);
}

class _ManejaAlimentacionState extends State<ManejaAlimentacion> {
  final Animal a;
  _ManejaAlimentacionState(this.a);
  final _formKey = GlobalKey<FormState>();
  var correoController = TextEditingController();
  DateTime? fechaIngreso = DateTime.now();
  late Future<List<PlanAlimentacion>> lc;

  @override
  initState(){
    super.initState();
    lc= API.getPlanesAlimentacion(a.id!);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text("Alimentación para ${a.nombre}", style: TextStyle(color: Colors.white, fontFamily: 'Nunito') ),
                          ...
                          builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                            if(snapshot.hasData){
                              if(snapshot.requireData.length==0) return Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text("No hay planes de alimentación registrados.", style:TextStyle(color: Colors.grey)),
                              );
                                  ...
                                  children: snapshot.data.map<Widget>(
                                      (l) => Card(
                                        ...
                                        child: (
                                        ExpansionTile(leading:Icon(LineIcons.drumstickWithBiteTakenOut),
                                          ...
                                          children:[
                                            ...
                                            ElevatedButton(child: Row(mainAxisSize: MainAxisSize.min,
                                              ...
                                              bool valor= await API.deletePAlimentacion(l.idPlan!);
                                              if(valor){
                                                final snackbar = SnackBar(content: Text("Alimentación eliminada."));
